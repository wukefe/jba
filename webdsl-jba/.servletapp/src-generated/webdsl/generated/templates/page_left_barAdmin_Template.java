...
public @SuppressWarnings("all") class page_left_barAdmin_Template extends TemplateServlet 
{ 
  ...
  { 
    ...
    { 
      if(i == 1)
      { 
        java.util.ArrayList<webdsl.generated.domain.Admin> elems20 = new java.util.ArrayList<webdsl.generated.domain.Admin>();
        for(org.webdsl.WebDSLEntity ent : elems)
        { 
          if(ent.instanceOf("Admin"))
          { 
            elems20.add((webdsl.generated.domain.Admin)ent);
          }
        }
      }
    }
  ...
  { 
    page_left_barAdmin_s_ad0String_MyArticle_Admin_Template.loadTemplateMap(staticEnv);
  }

  public static void loadRefArgClasses(Map<String, Class<?>> refargclasses)
  { 
    page_left_barAdmin_s_ad0String_MyArticle_Admin_Template.loadRefArgClasses(refargclasses);
  }
  ...
  { 
    staticEnv.putTemplate(keyOverwrite == null ? "page_left_barAdmin" : keyOverwrite, page_left_barAdmin_Template.class);
  ...
  { 
    return "template page_left_bar(admin : Admin)";
  }
  ...
  { 
    ...
    if(webdsl.generated.functions.genpagepredicate_table_content0Admin_.genpagepredicate_table_content0Admin_(admin_) || webdsl.generated.functions.inEmailContext_.inEmailContext_())
    { 
      if(RENDER_PHASE == phase)
      { 
        ...
        { 
          out.print("<a href=\"");
          out.print(utils.HTMLFilter.filter(utils.Navigate.constructUrl("table_content", utils.CollectionType.addAll(new java.util.ArrayList<String>(1), utils.URLFilter.filter(admin_.getNaturalId())), "Admin")));
          out.print("\"");
          { 
            StringBuilder classattrs = new StringBuilder();
            StringBuilder styleattrs = new StringBuilder();
    ...
    try
    { 
      handleTemplateCall(phase, inForLoop, forelementcounter, "tcall75", "wrapsubmitString", env.addExtraLocalTemplateArgumentsToArguments(new Object[]{"page_left_barAdmin_signout1" + webdsl.generated.functions.getPage_.getPage_().getTemplateContext().getTemplateContextString()}, "wrapsubmitString"), new Environment(env).putWithcall("s", new utils.TemplateCall("page_left_barAdmin_s_ad0String_MyArticle_Admin", getElementsContext(), new Object[]{new RefArg_page_left_barAdmin_Template0(page_left_barAdmin_Template.this), admin_}, attrs)), "wrapsubmitString", TemplateCall.EmptyAttrs);
    }
    catch(RuntimeException ex)
    { 
      printTemplateCallException(ex, "wrapsubmitString(\"page_left_barAdmin_signout1\" +\ngetPage().getTemplateContext().getTemplateContextString()) at unknown context");
    }
    if(RENDER_PHASE == phase)
    { 
      out.print("</form>");
      ...
      out.write(sw9.toString());
      if(threadLocalPageCached.inSubmittedForm() && threadLocalPageCached.isValidationFormRerender())
      { 
        ThreadLocalOut.pop();
        if(threadLocalPageCached.submittedFormContent == null)
        { 
          threadLocalPageCached.submittedFormContent = sw10.toString();
        }
        else
        { 
          out = ThreadLocalOut.peek();
